...
/**
 * Custom hook for fetching and processing data previews
 */
export function useDataFetching({ onDataFetched, setError }: UseDataFetchingOptions) {
    ...
    const fetchDataPreview = async (url: string, jsonPaths: string[], httpHeaders: string[] = []) => {
        setError(null);
        setLoading(true);
